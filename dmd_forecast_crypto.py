...
#%%
# Plotting functions
# Plot the average success matrix for all combinations
def plot_average_success_rate(average_success_matrix, higher_average, timeframe):
    column_labels = list(range(1,average_success_matrix.shape[0] + 1))
    columns = average_success_matrix.shape[1]
    row_labels = list(range(1,average_success_matrix.shape[1] + 1))
    rows = average_success_matrix.shape[0]
    fig, ax = plt.subplots(figsize=(average_success_matrix.shape[0] / 2, average_success_matrix.shape[1] / 2))
    im, cbar = heatmap(average_success_matrix.T, row_labels, column_labels)
    texts = annotate_heatmap(im, valfmt="{x:.2f}")
    ax.set_title(f"Prediction matrix for all pairs averaged\n Timeframe: {timeframe}")
    ax.set_xlabel("input steps")
    ax.xaxis.set_label_position('top') 
    ax.set_ylabel("future steps")           
    if higher_average["average"] > 0:
        rect = Rectangle(xy=(higher_average["input"] - 1 -0.5, higher_average["future"] - 1 - 0.5),
                         width=1,
                         height=1,
                         facecolor='None',
                         edgecolor='r',
